...
class FileBundler:
    ...
    def write_bundle(
        ...
        """Write discovered files to a single bundle .txt file.

        Returns (file_count, total_bytes, unmatched).
        """
        discovered, unmatched = self.discover_files(paths, file_type)
        base = base_dir or Path.cwd()
        ...
        for ft_name, file_paths in discovered.items():
            for fpath in sorted(file_paths):
                ...
                if ft_name == "notebooks":
                    content = self._read_notebook(fpath)
                else:
                    try:
                        content = fpath.read_text(errors="replace")
                    except OSError:
        ...
        header = (
            "# Source Code Bundle (generated by genai-cli)\n"
            f"# Files: {file_count}\n"
            "#\n"
            "# Each section starts with: ===== FILE: <relative_path> =====\n"
            "# To edit these files, use SEARCH/REPLACE blocks with the relative path.\n"
        )

        body = "\n\n".join(parts)
        full_content = header + "\n" + body + "\n" if body else header
        output.write_text(full_content, encoding="utf-8")
        total_bytes = len(full_content.encode("utf-8"))

        return file_count, total_bytes, unmatched

    def _read_notebook(self, fpath: Path) -> str:
        """Read a notebook file and return its cell content as text."""
        import nbformat

...
class TestWriteBundle:
    """Tests for write_bundle() disk output."""

    def test_creates_file_on_disk(
        self, bundler: FileBundler, sample_project_dir: Path, tmp_path: Path
    ) -> None:
        output = tmp_path / "bundle.txt"
        file_count, total_bytes, unmatched = bundler.write_bundle(
            [str(sample_project_dir / "src")], output, base_dir=sample_project_dir,
        )
    ...
    def test_header_format(
        ...
        assert content.startswith("# Source Code Bundle (generated by genai-cli)")
    ...
    def test_relative_paths_only(
        ...
        # Should NOT contain absolute paths in FILE markers
        assert str(sample_project_dir) not in content.split("===== FILE:")[1].split("=====")[0]
    ...
    def test_unmatched_reporting(
        ...
        output = tmp_path / "bundle.txt"
        file_count, total_bytes, unmatched = bundler.write_bundle(
            [str(tmp_path / "nonexistent.py")], output,
        )
        assert file_count == 0
        assert len(unmatched) == 1

    def test_handles_notebooks(
        self, bundler: FileBundler, tmp_path: Path
    ) -> None:
        nb = {
            "nbformat": 4,
        ...
        nb_path.write_text(json.dumps(nb))
        output = tmp_path / "bundle.txt"
        file_count, _, _ = bundler.write_bundle(
            [str(nb_path)], output, base_dir=tmp_path,
        )
        assert file_count == 1
        content = output.read_text()
        assert "===== FILE: test.ipynb =====" in content
    ...
    def test_file_type_filter(
        ...
        output = tmp_path / "bundle.txt"
        file_count, _, _ = bundler.write_bundle(
            [str(sample_project_dir)], output,
            file_type="code", base_dir=sample_project_dir,
        )
        content = output.read_text()
        assert "README.md" not in content
        assert file_count >= 2  # main.py, utils.py, test_main.py
